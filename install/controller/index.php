		...
		if ($this->request->post) {
			...
			if ($isRootPublic && $adminPath) {
				...
				if (@rename($from, $to)) {
					...
					$error = sprintf(__('Renaming admin login path from %s to %s failed! use /admin path to login'), 'vadmin', $adminPath);
				}
			}
			...
			$location              = preg_replace('@/install.*$@', $admin_path . "?success=$success&error=$error", ($_SERVER['REQUEST_URI'] ?? ''));
